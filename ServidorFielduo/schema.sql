...
CREATE TABLE IF NOT EXISTS solicitacoes_materiais (id INT AUTO_INCREMENT PRIMARY KEY, empresa_id INT NOT NULL, tecnico_id INT NOT NULL, cliente_id INT NOT NULL, material_id INT NOT NULL, quantidade DECIMAL(10,2) NOT NULL, unidade VARCHAR(20) NOT NULL, status VARCHAR(30) NOT NULL DEFAULT 'SOLICITADO', observacao VARCHAR(255) NULL, criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, atendido_em DATETIME NULL, INDEX idx_solicitacoes_empresa_tecnico (empresa_id,tecnico_id,criado_em), INDEX idx_solicitacoes_empresa_cliente (empresa_id,cliente_id), INDEX idx_solicitacoes_status (empresa_id,status)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
